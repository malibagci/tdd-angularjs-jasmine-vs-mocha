...
Der Nachteil von Integration-Tests ist, dass diese im Vergleich zu Unit-Tests eher langsam sind. Mit Integration-Tests können dafür Verbindungen und Abhängigkeiten zwischen einzelnen Komponenten getestet werden. 
...
Dieser Test (Listing \ref{listing:tdd-simple-addition}) wird per Kommandozeile im Browser \glqq{PhantomJS\grqq} ausgeführt. Der Befehl dafür lautet: \glqq{karma start karma.conf.js\grqq}. Der erste Test ist geschrieben, jedoch gibt es noch keine Implementierung, dass heißt hier wird ein negatives Testergebnis erwartet. Das erwartete Ergebnis ist auf Abbildung \ref{figure:tdd-simple-step-1-1} zu sehen.
...
Wie die bisherigen Beispiele für \glqq{Fake it\grqq} und \glqq{Triangulate\grqq} zeigen, handelt es sich dabei um wirklich kleine Schritte. Speziell anhand dieses Beispiels lässt sich das dritte Green Bar Pattern erklären: \glqq{Obvious Implementation\grqq}.
...
Listing \ref{listing:angular-services} verdeutlicht die Funktionsweise und Verwendung von Services:
...
Wenn nun der \glqq{FooCtrl\grqq} Controller \glqq{\$scope.fooBarService.fooBar = true\grqq} aufruft, so wird automatisch der \$scope des \glqq{BarCtrl\grqq} Controllers aktualisiert.
...
.2 app.
.3 bower\_components\DTcomment{notwendigen Frontend Bibliotheken (z.B. Angular)}.
